...
void rosc_set_dormant(uint32_t mode) {
    // mode ROSC_DORMANT_VALUE_WAKE or ROSC_DORMANT_VALUE_DORMANT
    // see hardware/regs/rosc.h
    if (mode == ROSC_DORMANT_VALUE_DORMANT) {
        // check if wake irqs must be set.
        if (gpio_enabled_dormant_irqs()) {
            // WARNING: This stops the rosc until woken up by an irq
            rosc_write(&rosc_hw->dormant, mode);
        } else
            mp_warning("No Wake IRQs have been set - Dormant mode not entered");
